...
/**
 * my_is_cmd - Determines whether a file can be executed as
 * a command
 *
...
/**
 * my_dupli_chars - duplicates characters.
 ...
 *
 */
char *my_dupli_chars(char *string_path, int stop_index, int start_index)
{
	int d = 0;
...
char *my_get_path(info_t *info, char *command, char *string_path)
{
	...
	if (!string_path)
		return (NULL);
	if ((my_strlen(command) > 2) && my_starts_with(command, "./"))
	{
		if (my_is_cmd(info, command))
			return (command);
	}
	while (1)
	{
		...
		{
			path12 = my_dupli_chars(string_path, cur_posi, p);
			if (!*path12)
				my_strcat(path12, command);
			else
			{
				my_strcat(path12, "/");
				my_strcat(path12, command);
			}
			if (my_is_cmd(info, path12))
				return (path12);
			if (!string_path[p])
				break;
			cur_posi = p;
